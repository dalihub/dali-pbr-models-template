...
void main()
{
  ...
  vec4 metalRoughness = vec4(1.0);
  vec4 normalRoughness = texture(sNormal, vUV.st);
  vec3 normalMap = normalRoughness.rgb;
  float roughness = normalRoughness.a;

  vec4 albedoAlpha = texture(sAlbedoAlpha, vUV.st);
  vec3 albedoColor = vec3(1.0) + 0.0001*albedoAlpha.rgb;
  float metallic = metalRoughness.r;
  float alpha = albedoAlpha.a;
roughness =albedoAlpha.r;
  //Normalize vectors
  ...
  //FragColor = vec4( finalColor/2.0, alpha + alpha*0.0001 );
FragColor = vec4( 4.0*finalColor - vec3(3.0), alpha + alpha*0.0001 );
}
